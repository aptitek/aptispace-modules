...
_ramStorageTabWatcher = {
  const watcher = aptitek.createTabsetWatcher(
    ".ram-storage-tabset",
    { "Stockage en ligne": "ligne", "Stockage en colonne": "colonne" },
    (value) => { mutable storageMode = value; }
  );
  invalidation.then(() => watcher.destroy());
  return null;
}

ramTableData = aptitek.parseTableData("#ram-col-data-source table")

renderRamGraph = {
  const graph = aptitek.createRamStorageGraph("ram-col-graph-container", storageMode, "Salaire", ramTableData, { autoStart: false });
  invalidation.then(() => graph?.destroy?.());
  return graph;
}

ramPlaybackControl = aptitek.createSimulationControl("#ram-col-controls", {
