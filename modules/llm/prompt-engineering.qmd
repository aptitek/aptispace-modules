...
:::: {.row gap=4}
::::: {.col}
[Température (T)]{.slider #temp-val value=0.7 min=0.01 max=2 step=0.05}
:::::
::::: {.col}
[Top-K]{.slider #top-k-val value=8 min=1 max=8 step=1}
:::::
::::: {.col}
[Top-P (Nucleus)]{.slider #top-p-val value=1 min=0.1 max=1 step=0.05}
:::::
::::
...
renderDistrib = {
  ...
  createBar("plot-distrib", {
    x: items.map(d => d.word),
    y: items.map(d => +(d.prob * 100).toFixed(1))
  }, "", {
    marker: {
      color: items.map(d =>
